function push_data_to_gradio_component(DAT,ELEM_ID,TYPE){if(TYPE=="str"){}
else if(TYPE=="obj"){}
...
document.body.removeChild(textArea);}}catch(error){console.error("Copy failed: ",error);}});if(enable_tts){var audioButton=document.createElement('button');audioButton.classList.add('audio-toggle-btn');audioButton.innerHTML=audioIcon;audioButton.addEventListener('click',async()=>{if(audioPlayer.isPlaying){allow_auto_read_tts_flag=false;toast_push('自动朗读已禁用。',3000);audioPlayer.stop();setCookie("js_auto_read_cookie","False",365);}else{allow_auto_read_tts_flag=true;toast_push('正在合成语音 & 自动朗读已开启 (再次点击此按钮可禁用自动朗读)。',3000);const readText=botElement.innerText;push_text_to_audio(readText);setCookie("js_auto_read_cookie","True",365);}});}
...
function chatbotContentChanged(attempt=1,force=false){for(var i=0;i<attempt;i++){setTimeout(()=>{const messages=gradioApp().querySelectorAll('#gpt-chatbot .message-wrap .message.bot');messages.forEach((message,index,arr)=>{const is_last_in_arr=index===arr.length-1;addCopyButton(message,index,is_last_in_arr);});},i===0?0:200);}}
...
enable_tts=false;async function GptAcademicJavaScriptInit(dark,prompt,live2d,layout,tts){audio_fn_init();minor_ui_adjustment();chatbotIndicator=gradioApp().querySelector('#gpt-chatbot > div.wrap');var chatbotObserver=new MutationObserver(()=>{chatbotContentChanged(1);});chatbotObserver.observe(chatbotIndicator,{attributes:true,childList:true,subtree:true});if(layout==="LEFT-RIGHT"){chatbotAutoHeight();}
...
function reset_conversation(a,b){console.log("js_code_reset");a=btoa(unescape(encodeURIComponent(JSON.stringify(a))));setCookie("js_previous_chat_cookie",a,1);gen_restore_btn();return[[],[],"已重置"];}
function restore_previous_chat(){console.log("restore_previous_chat");let chat=getCookie("js_previous_chat_cookie");chat=JSON.parse(decodeURIComponent(escape(atob(chat))));push_data_to_gradio_component(chat,"gpt-chatbot","obj");document.querySelector("#elem_update_history").click();}
...
prev_text="";prev_text_already_pushed="";prev_chatbot_index=-1;const delay_live_text_update=trigger(3000,on_live_stream_terminate);function on_live_stream_terminate(latest_text){console.log("on_live_stream_terminate",latest_text)
remaining_text=latest_text.slice(prev_text_already_pushed.length);if((!isEmptyOrWhitespaceOnly(remaining_text))&&remaining_text.length!=0){prev_text_already_pushed=latest_text;push_text_to_audio(remaining_text);}}
...
else if(chatbot_index==prev_chatbot_index&&!is_continue){console.log('---------------------')
console.log('text twisting!')
console.log('[new message begin]','text',text,'prev_text_already_pushed',prev_text_already_pushed)
console.log('---------------------')
prev_text_already_pushed="";delay_live_text_update(text);}
else{console.log('---------------------')
console.log('new message begin!')
console.log('[new message begin]','text',text,'prev_text_already_pushed',prev_text_already_pushed)
console.log('---------------------')
prev_text_already_pushed="";process_increased_text(text);delay_live_text_update(text);}
...
await audio_push_lock.lock();var lines=text.split(/[\n。]/);for(const audio_buf_text of lines){if(audio_buf_text){const url=`${window.location.href}vits`;const payload={text:audio_buf_text,text_language:"zh"};post_text(url,payload,send_index);send_index=send_index+1;console.log(send_index,audio_buf_text)
if(allow_auto_read_tts_flag){await delay(3000);}}}
audio_push_lock.unlock();}
send_index=0;recv_index=0;to_be_processed=[];async function UpdatePlayQueue(cnt,audio_buf_wave){if(cnt!=recv_index){to_be_processed.push([cnt,audio_buf_wave]);console.log('cache',cnt);}
else{console.log('processing',cnt);recv_index=recv_index+1;if(audio_buf_wave){audioPlayer.enqueueAudio(audio_buf_wave);}
while(true){find_any=false;for(i=to_be_processed.length-1;i>=0;i--){if(to_be_processed[i][0]==recv_index){console.log('processing cached',recv_index);if(to_be_processed[i][1]){audioPlayer.enqueueAudio(to_be_processed[i][1]);}
