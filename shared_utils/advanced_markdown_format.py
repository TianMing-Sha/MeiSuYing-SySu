...
class CustomHeaderExtension(Extension):
    def extendMarkdown(self, md):
        md.preprocessors.register(HeaderPreprocessor(md), 'custom_header', 27)
...
class HeaderPreprocessor(Preprocessor):
    def run(self, lines):
        ...
        for line in lines:
            ...
            if match:
                level = len(match.group(1))  # Determine the header level based on the number of #
                text = match.group(2).strip()
                new_line = f'<h{level} class="custom-header-class">{text}</h{level}>'
